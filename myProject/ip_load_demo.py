...
# 备注： 该程序只作为校园网一键登录app，需要提前装好tkinter&win32模块 当然也可联系我直接拿app，我在这里将源码分享 ---作者qq：780988326
...
class App:
    ...
    def read(self):
        with open(os.getcwd()+'\\document-link\\user-agent.txt','r') as f:
            struser = f.read()
            print(struser)
            r = re.search('username:(\d*)password:(.*)mac:(.*)statu:(.*)select:(.*)', struser)
...
def link(username, password, mac, select):
    ...
    headers = {
            ...
            'Referer': 'POST http://172.22.0.13/webauth.do?wlanuserip=' + get_ip() + '&wlanacname=NFV-VBRAS-0' + select + '&mac=' + mac +'&vlan=1056&rand=12b70fb2155820&url=http://edge.microsoft.com/generate_20',
